...
* 👯 Calculated property, which gets called only when the dependent property gets changed
* ✂️ Commit mass mutation by acquiring locking
...
### `static create`
Takes an simple JavaScript Object and make it observable. This returns the new object as a state instance.
...
```javascirpt
myState.create('range.type', { absolute: true });
```
...
```
If myState.seriaize() is called the following output is shown
```javascript
// Output
{
    range: {
...
myState.prop('range.start', 12); // Output
Start [10, 12]
End [5, 5]
myState.prop('range.end', 7); // Output
Start [12, 12]
End [5, 7]
```
...
// Output Start [10, 12] End [5, 7]
```
Once `lock()` is called the state caches all the change that comes afer this. When `unlock()` is called it applies all the changes to the state and the handler is called.
